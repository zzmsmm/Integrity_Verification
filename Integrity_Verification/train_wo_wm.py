...
parser.add_argument('--fpmethod', default='none', help='use for fingerprint model')
...
try:
    ...
    if args.fpmethod == 'none':
        log_dir = os.path.join(cwd, 'log', str(args.dataset), str(args.arch), 'clean')
    else:
        log_dir = os.path.join(cwd, 'log', str(args.dataset), str(args.arch), 'fingerprint', str(args.fpmethod))
    ...
    if args.fpmethod == 'none':
        ...
        save_dir = os.path.join(cwd, 'checkpoint', str(args.dataset), str(args.arch), 'fingerprint', str(args.fpmethod))
